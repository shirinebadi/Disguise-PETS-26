...
// #define BAD_CALL(fn) void fn(void){ocall_print_string(#fn);EAPP_RETURN(EAPP_ERR_RET);}
...
//   ocall_print_string("NOT USING REAL RANDOMNESS: TEST ONLY\n");
 ...
 */
#define EAPP_ERR_RET 5
#define BAD_CALL(fn) void fn(void){EAPP_RETURN(EAPP_ERR_RET);}
...
// void* memset(void* s, int c, size_t n) __attribute__((noinline));
...
int gettimeofday(struct timeval *__restrict tv, void *__restrict tz) {
    ...
    // Assuming CPU frequency is known (e.g., 1GHz = 1,000,000,000 cycles per second)
    const unsigned long CPU_FREQ = 1000000000;  // Replace with actual CPU frequency
    
    tv->tv_sec = cycles / CPU_FREQ;  // Integer division for seconds
    tv->tv_usec = ((cycles % CPU_FREQ) * 1000000) / CPU_FREQ;  // Remaining cycles to microseconds
